...
French translation of `ecommerce-scraping-apis`, published as `github.com/automationbyexperts/apis-scraping-e-commerce-france`
(local folder `all_public_actors/APIFY_GITHUB/apis-scraping-e-commerce-france`). Same affiliate model: every Apify link
carries `?fpr=youssef`. Read `ecommerce-scraping-apis/CLAUDE.md` for how the build works; everything there applies here.
...
- A regional group sits **first** in `groups` (`france-francophonie`: Leboncoin, Cdiscount, Fnac et boutiques françaises), so a local tool is
